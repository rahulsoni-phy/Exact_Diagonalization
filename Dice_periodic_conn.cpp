...
//-----------------------------------------------//
int r, r1, r2, r3, r4, r5, r6, ry, rx;
Mat_Size=6*Lx*Ly;
Mat_Half_Size=3*Lx*Ly;
hop=1.0;
cout<<"This code generates OBCxOBC long-range Dice connections"<<endl;
cout<<"Total number of unit cells in the lattice ="<<Lx*Ly<<endl;
cout<<"Total number of sites in the lattice ="<<3*Lx*Ly<<endl;
cout<<"Rashba-SOC strength ="<<l_coup<<endl;
cout<<"Magnetic field strength on site coordination-3="<<B_mag<<endl;
cout<<"Magnetic field strength on site coordination-6="<<B1_mag<<endl;
...
for(int ap=0;ap<3;ap++){
	...
	for(int iy=0;iy<Ly;iy++){
		...
		r1 = sp*MHS + (ap-1 + 3*ix     + 3*Lx*iy );
		r2 = sp*MHS + (ap+1 + 3*ix     + 3*Lx*iy );
		r3 = sp*MHS + (ap-1 + 3*ix     + 3*Lx*(iy+1) );
		r4 = sp*MHS + (ap-1 + 3*(ix+1)     + 3*Lx*iy );
		r5 = sp*MHS + (ap+1 + 3*(ix-1) + 3*Lx*iy );
		r6 = sp*MHS + (ap+1 + 3*ix     + 3*Lx*(iy-1) );
        ...
        //---------------Implement PBC-Y in Tight-Binding Term------------------//
                if(iy==Ly-1){
                        if(ap!=0){
                                ry = sp*MHS + (ap-1 + 3*ix );

                                C_mat[r][ry].real( (-1.0)*hop );
                                C_mat[r][ry].imag(0.0);
                                C_mat[ry][r].real( (-1.0)*hop );
                                C_mat[ry][r].imag(0.0);
                        }
                }

	//---------------Implement PBC-X in Tight-Binding Term------------------//
                if(ix==Lx-1){
                        if(ap!=0){
                                rx = sp*MHS + (ap-1 + 3*Lx*iy );

                                C_mat[r][rx].real( (-1.0)*hop );
                                C_mat[r][rx].imag(0.0);
                                C_mat[rx][r].real( (-1.0)*hop );
                                C_mat[rx][r].imag(0.0);
                        }
                }
...
for(int j=0;j<MHS;j++){
if(C_mat[j][i].real()!=0){
cout<<i<<"	"<<"to"<<"	"<<j<<"		"<<C_mat[j][i].real()<<endl;
}
}
}
*/
...
for(int ap=0;ap<3;ap++){
		...
		if(ap==1){
	        ...
	        //---------------------Implement PBC-Y in Rashba-SOC Term-------------------//
                if(iy==Ly-1){
                ry=(ap-1 + 3*ix );

                        if(ap==1){
                        C_mat[ry][r+MHS].real( (1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[ry][r+MHS].imag( (-1.0)*l_coup*cos( (2.0*PI)/3.0 ) );
                        C_mat[r+MHS][ry].real( (1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[r+MHS][ry].imag( (1.0)*l_coup*cos( (2.0*PI)/3.0 ) );

                        C_mat[ry+MHS][r].real( (-1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[ry+MHS][r].imag( (-1.0)*l_coup*cos( (2.0*PI)/3.0 ) );
                        C_mat[r][ry+MHS].real( (-1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[r][ry+MHS].imag( (1.0)*l_coup*cos( (2.0*PI)/3.0 ) );
                        }
		...
		//---------------------Implement PBC-X in Rashba-SOC Term-------------------//
                if(ix==Lx-1){
                rx = (ap-1 + 3*Lx*iy );

                        if(ap==1){
                        C_mat[rx][r+MHS].real( (-1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[rx][r+MHS].imag( (-1.0)*l_coup*cos( (2.0*PI)/3.0 ) );
                        C_mat[r+MHS][rx].real( (-1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[r+MHS][rx].imag( (1.0)*l_coup*cos( (2.0*PI)/3.0 ) );

                        C_mat[rx+MHS][r].real( (1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[rx+MHS][r].imag( (-1.0)*l_coup*cos( (2.0*PI)/3.0 ) );
                        C_mat[r][rx+MHS].real( (1.0)*l_coup*sin( (2.0*PI)/3.0 ) );
                        C_mat[r][rx+MHS].imag( (1.0)*l_coup*cos( (2.0*PI)/3.0 ) );
                        }
...
/*
for(int i=MHS;i<2*MHS;i++){
for(int j=0;j<MHS;j++){
if(C_mat[j][i].imag()!=0){
cout<<i-MHS<<"      "<<"to"<<"      "<<j<<"         "<<C_mat[j][i]<<endl;
}
}
}
*/
...
double B_val,RSOC_val;

B_val=B_mag;
RSOC_val=l_coup;

connec_out<<RSOC_val<<" "<<B_val<<endl;
